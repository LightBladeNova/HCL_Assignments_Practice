...
def usagestats():
	...
	usagestats.close()
	return current_foreground_package
...
if __name__ == '__main__':
	with open("android_data.csv", "w") as file:
		...
		while time.time() < timeout_start + time_to_run_in_seconds:
			time.sleep(0.05)
			current_foreground_package = usagestats()
			cpuinfo_totals = cpuinfo()
			diskstats_all = diskstats()
			meminfo_RAM = meminfo()
			battery_all = battery()
			batterystats_all = batterystats()
			wifi_info = wifi()
			mWifiInfo_read = mWifiInfo()
			mSignalStrength_read = mSignalStrength()
